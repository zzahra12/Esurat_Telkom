...
import '../services/gno_pdf_service.dart';
import 'preview_surat_page.dart';

class FormGnoPage extends StatefulWidget {
  const FormGnoPage({super.key});

  @override
  State<FormGnoPage> createState() => _FormGnoPageState();
}
      ...
      Navigator.push(
        ...
        MaterialPageRoute(
          builder: (context) => PreviewSuratPage(
            jenisSurat: 'Surat Permintaan Ganti Nomor (GNO)',
            pdfBytes: pdfBytes,
            fileName: fileName,
            onCetakPdf: (bytes, name) async {
              await GnoPdfService.saveAndOpenFile(bytes, name);
            },
          ),
        ),
      );
